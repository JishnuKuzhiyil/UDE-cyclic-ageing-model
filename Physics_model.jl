...
include("SPM_Model_Aux.jl") ; import .SPM_Model_Aux # import functions for SPM model
include("Experiment.jl"); import .Experiment  # import functions for defining the experiment
include("Deg_model_Aux.jl"); import .Deg_model_Aux # import functions for degradation model
...
Condition_index = 1 #Index of the condition to be simulated

Condition_name = condition_names[Condition_index]
V_min, Volt_hold, V_max, Dis_SOC, Ch_SOC, tol, N_cyc_bet_RPT, N_RPTs, Ch_I, Dis_I, T∞ = ExperimentConditions.get_test_condition(Condition_name)
title_cond=ExperimentConditions.get_title_for_condition(Condition_name)
...
Exp_RPT = Experiment.Experiment.RPT_cycle # RPT
...
Deg_Para = (K_SEI_ref,Ea_K_SEI,D_SEI_ref,Ea_D_SEI,K_LiP_ref,Ea_K_LiP,β_pore_ref,Ea_β_pore,β_neg_ref,Ea_β_neg,m_neg,M_σ_neg,β_pos_ref,Ea_β_pos,m_pos,M_σ_pos,k_cr,m_cr)
...
function TSPMD_model!(du, u, p, t)
    ...
    K_SEI_ref,Ea_K_SEI,D_SEI_ref,Ea_D_SEI,K_LiP_ref,Ea_K_LiP,β_pore_ref,Ea_β_pore,β_neg_ref,Ea_β_neg,m_neg,M_σ_neg,β_pos_ref,Ea_β_pos,m_pos,M_σ_pos,k_cr,m_cr = p[4]
    ...
    β_neg = β_neg_ref * exp(-Ea_β_neg/8.3*(1/T - 1/298.15))
    β_pos = β_pos_ref * exp(-Ea_β_pos/8.3*(1/T - 1/298.15))

     #Degradation equations__________________________________________________________________

    #Anode and cathode potential calculation (anode for thermal and degradation dynamics, cathode for thermal dynamics)
    c_n_surf = 1.5 * u[6] - 0.5 * u[5] # surface concentration at anode (Linear extrapolation between nodes)
    c_p_surf = 1.5 * u[12] - 0.5 * u[11] #surface concentration at cathode  (Linear extrapolation between nodes)
    c_n_avg = SPM_Model_Aux.FVM_vol_avg(c_n) # Average anode concentration
    c_p_avg = SPM_Model_Aux.FVM_vol_avg(c_p) #Average cathode concentration
    ΔC_n = c_n_surf  - c_n_avg 
    ΔC_p = c_p_surf - c_p_avg

    j_n = I / (Para_set.A * Para_set.Ln) / bn #Anode interfacial current density [A/m²]
    j_n_0 = Para_set.Negative_exchange_current_density(c_n_surf, T) # Anode exchange current density [A/m²]
    ηₙ = Para_set.Negative_OCP(c_n_surf) + (2 * Para_set.R * T / Para_set.F) * asinh(j_n / j_n_0) #Anode potential
    ηₙ_over = (2 * Para_set.R * T / Para_set.F) * asinh(j_n / j_n_0)#Anode overpotential 

    j_p = -I / (Para_set.A * Para_set.Lp) / bp #Cathode interfacial current density [A/m²]
    j_p_0 = Para_set.Positive_exchange_current_density(c_p_surf, T)# Cathode exchange current density [A/m²]
    ηₚ_over = (2 * Para_set.R * T / Para_set.F) * asinh(j_p / j_p_0) #Cathode overpotential

    η_reaction =  ηₚ_over - ηₙ_over #full cell reaction overpotential
    R0 = 0.00022
    ...
    #SEI calculations
    η_SEI =  ηₙ - Deg_model_Aux.U_SEI #SEI potential
    J_SEI = Deg_model_Aux.J_SEI_eval(δ_SEI,η_SEI,T,D_SEI,K_SEI)
    du[13] = 0.5*J_SEI*Deg_model_Aux.V_SEI

    #LiP calculations
    J_LiP = Deg_model_Aux.J_LIP_eval(ηₙ,T,ΔC_n,K_LiP)
    du[14] = J_LiP*Deg_model_Aux.V_LiP

    #LAM calculations (Mechanical effects are updated at end of each cycle)
    du[15] =   - β_pore * (Para_set.F*J_SEI *bn )*3/Para_set.Rn
    du[16] = 0.0

     #Thermal dynamics________________________________________________________________________
     Q_gen = abs(I*( η_reaction + η_ohmic) ) # [W]        
     du[17] = (Para_set.h*Para_set.Cooling_Area*(T∞_curr - T) + Q_gen)/(Para_set.Vol*Para_set.Cp) # [K/s] # 0 dimensional thermal model

      #Parrticle cracking and fracturing________________________________________________________
      l_cr,a_cr= u[18],u[19]
      ...
      du[21] = abs(I)/3600.0 # State variable to track Abs throughput of the cell
      du[22] =  β_pore * (Para_set.F*J_SEI *bn )/bn #State variable to keep track of pore blockage LAM

      #Flux at particle surface.
      N_surf_neg = j_n/Para_set.F +J_SEI + J_LiP + J_SEI_cr* (a_cr/bn) # [mol/m²/s] # change this factor laster in the ρ_crack
      N_surf_pos = j_p/Para_set.F # [mol/m²/s]
       ...
       if p[3][step_nu].is_voltage_hold
           c_n_surf,c_p_surf = 1.5 * u[6] - 0.5 * u[5], 1.5 * u[12] - 0.5 * u[11]
...
function condition(u, t, integrator)
    ...
    if Exp_step.is_voltage_hold #If the ongoing step is voltage hold
        ...
        if Exp_step.Termination_condition == "Voltage"
            ...
            c_n_surf,c_p_surf = 1.5 * u[6] - 0.5 * u[5], 1.5 * u[12] - 0.5 * u[11]
            return SPM_Model_Aux.SPM_voltage(c_n_surf,c_p_surf, bn, bp, δ_SEI,δ_LIP, I, u[17]) - Exp_step.Termination_val
        ...
        elseif Exp_step.Termination_condition == "Dis_SOC"
            ...
            c_n_surf,c_p_surf = 1.5 * u[6] - 0.5 * u[5], 1.5 * u[12] - 0.5 * u[11]
            vol_condition= SPM_Model_Aux.SPM_voltage(c_n_surf,c_p_surf, bn, bp, δ_SEI,δ_LIP, I, u[17]) - 2.5
            return SOC_condn*vol_condition

        elseif Exp_step.Termination_condition == "Ch_SOC" #CC charging step with SOC termination
           
                Q_RPT_latest = integrator.p[end].Q_RPT[end]
                SOC_condn =  u[21]/Q_RPT_latest-(Exp_step.Termination_val)
                c_n_surf,c_p_surf = 1.5 * u[6] - 0.5 * u[5], 1.5 * u[12] - 0.5 * u[11]
                vol_condition= SPM_Model_Aux.SPM_voltage(c_n_surf,c_p_surf, bn, bp, δ_SEI,δ_LIP, I, u[17]) - 4.2
...
function affect!(integrator)
    ...
    #This function is called at the end of each experimental step (when the corresponding event condition is met). 
    Erase_sol = true # Erase solution at end of every experimental setp, this helps reduce memory consumption. Do not change as max and min stress is calculated based on this.

    skip_nu=0 #number of steps to skip, default is zero.  
    Step_nu = integrator.p[1][1]  # the step number which just finished
    t_step_start = integrator.p[2][1]  # the starting time of the step that finished
    t_step_end = integrator.t  # the ending time of the step that finished
    last_Ah=integrator.u[21] # absolute charge throughput during the step that just finished
    integrator.u[21]=0.0 #resetting the absolute charge throughput to zero for the next step

    if (integrator.p[3][Step_nu].extra == "Charge_step") && (integrator.p[3][Step_nu].Termination_condition == "Ch_SOC")

        #The experiment step that just finished is CC charge with SOC based termination
        if abs(last_Ah/integrator.p[end].Q_RPT[end] - integrator.p[3][Step_nu].Termination_val) < 0.001 # required SOC reached already (0.001 is put as a tolerance)
             skip_nu=1 # skip the next voltage hold phase as the required SOC is already reached
        else 
            integrator.u[21]=last_Ah # if the required SOC is not reached, the absolute charge throughput is restored to the value before resetting
        end
    ...
    if ((integrator.p[3][Step_nu].extra == "start_of_cycle") || (integrator.p[3][Step_nu].extra == "start_of_RPT")) 
        ...
        #Start of a new cycle or RPT
        cycle_num = integrator.p[6][1]
        integrator.p[6][1] +=1 # cycle number that just finished.

 

        #update anode anode active materialsurface area density based on stress based calculation.
        ΔC_max_neg= integrator.p[5][2]
        ΔC_min_neg = integrator.p[5][4]
        θ_soc0_neg = integrator.p[7][1]
        β_neg = β_neg_ref * exp(-Ea_β_neg/8.3*(1/integrator.u[17] - 1/298.15))
        integrator.u[15] +=  Deg_model_Aux.Stress_driven_LAM_neg(ΔC_min_neg,ΔC_max_neg,β_neg,m_neg,M_σ_neg)
        
       
        #update cathode active materialsurface area density based on stress based calculation.
        ΔC_max_pos = integrator.p[5][1]
        ΔC_min_pos = integrator.p[5][3]     
        β_pos = β_pos_ref * exp(-Ea_β_pos/8.3*(1/integrator.u[17] - 1/298.15))
        integrator.u[16] += Deg_model_Aux.Stress_driven_LAM_pos(ΔC_min_pos,ΔC_max_pos,β_pos,m_pos,M_σ_pos)
    ...
    if integrator.p[3][Step_nu].extra == "Charge_step"
        ...
        #Exp step that just finished is CC charge step
        #ΔC at anode takes the maximum value during charging step
        #ΔC at cathode takes the minimum value during charging step

        C_n_array = @view(integrator.sol[1:6,:])
        C_p_array = @view(integrator.sol[7:12,:])

        weights = [0, 0, 0, 0, -0.5, 1.5] .- ([1,7,19,37,61,91] ./ 216) # weights to get surface - average concentration        
    ...
    if integrator.p[3][Step_nu].extra == "Discharge_step"
        ...
        #Exp step that just finished is CC discharge step
        #ΔC at anode takes the minimum value during discharge step
        #ΔC at cathode takes the maximum value during discharge step

        C_n_array = @view(integrator.sol[1:6,:])
        C_p_array = @view(integrator.sol[7:12,:])

        weights = [0, 0, 0, 0, -0.5, 1.5] .- [1, 7, 19, 37, 61, 91] ./ 216
        ...
        integrator.p[7][1] = sum(([1, 7, 19, 37, 61, 91] ./ 216).*C_n_array[1:6,end])/Para_set.c_s_max_neg
        integrator.p[7][2] = sum(([1, 7, 19, 37, 61, 91] ./ 216).*C_p_array[1:6,end])/Para_set.c_s_max_pos
        ...
        if cycle_num==dis_voltage_cycle_num
              ...
              δ_SEI_vec = Dis_sol[13,:].+Dis_sol[19,:]; global T_vec = Dis_sol[17,:]
    ...
    if integrator.p[3][Step_nu].extra=="Capacity_mes_step"
        ...
        #taking measurements from RPT step
        ...
        Ntot = Ntot_anode + Ntot_cathode #Ah
    ...
    #Stop the simulation if the step finished is th elast experiment step
    ...
    # Change temperature to 25 degree before RPT and back to experimental temperature after RPT
    if integrator.p[3][Step_nu+1].extra=="start_of_RPT" #change temperature to 25 degree before rpt
            integrator.u[17]=298.15
            global T∞_curr = 298.15
        elseif integrator.p[3][Step_nu].extra=="Capacity_mes_step" # change temperature back to exp-temp after RPT
            integrator.u[17]=T∞
            global T∞_curr = T∞
    end
    ...
    reinit!(integrator, integrator.u, t0=t_step_end, tf=next_step_duration,erase_sol=Erase_sol) #reinitialise the integrator for the next step
...
# """___________________________________________Save data_________________________________________________________________________________________"""
...
function save_model_results(x, Capacity, LAM_anode, LAM_cathode, sim_state, Condition_name )
    ...
    file_name = "Output_data/Phy_stage3/results$(Condition_name).txt"
